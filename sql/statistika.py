...
def sestevek_letne_prodaje(poslovalnica, izbrano_leto):
  ...
  SELECT SUM(prodaja.kolicina * izdelki.prodajna_cena) FROM prodaja
...
def nabavna_cena_v_letu((poslovalnica, izbrano_leto):
  prodaja = f'prodaja{izbrano_leto}'
  SELECT SUM(prodaja.kolicina * izdelki.nabavna_cena) FROM prodaja
    JOIN izdelki ON izdelki.ime = prodaja.izdelek
      WHERE prodaja.id_poslovalnice = poslovalnica
...
def najboljsi_dan_prodaje(poslovalnica, izbrano_leto):
  prodaja = f'prodaja{izbrano_leto}'
  SELECT datum, SUM(prodaja.kolicina * izdelki.prodajna_cena) AS dnevni_prihodek FROM prodaja
    JOIN izdelki ON izdelki.ime = prodaja.izdelek
      WHERE prodaja.id_poslovalnice = poslovalnica
        GROUP BY datum
